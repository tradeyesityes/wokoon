...
Pong = {
  ...
  Images: [
    template_directory_uri+"/images/press1.png",
    template_directory_uri+"/images/press2.png",
    template_directory_uri+"/images/winner.png"
  ],
  ...
  initialize: function(runner, cfg) {
    Game.loadImages(Pong.Images, function(images) {
      ...
      this.backgroundNoise = Object.construct(Pong.BackgroundNoise, this);
  ...
  Menu: {
    ...
    initialize: function(pong) {
      var press1 = pong.images[template_directory_uri+"/images/press1.png"];
      var press2 = pong.images[template_directory_uri+"/images/press2.png"];
      var winner = pong.images[template_directory_uri+"/images/winner.png"];
  ...
  Sounds: {
    ...
    initialize: function(pong) {
      ...
      if (this.supported) {
        this.files = {
          ping: Game.createAudio(template_directory_uri+"/sounds/ping.wav"),
          pong: Game.createAudio(template_directory_uri+"/sounds/pong.wav"),
          wall: Game.createAudio(template_directory_uri+"/sounds/wall.wav"),
          goal: Game.createAudio(template_directory_uri+"/sounds/goal.wav")
        };
      }
    },
    ...
    ping: function() {
      this.play('ping');
      this.game.backgroundNoise.ping = false;
    },
    pong: function() {
      this.play('pong');
      this.game.backgroundNoise.ping = true;
    },
  ...
  Paddle: {
    ...
    initialize: function(pong, rhs) {
      ...
      this.minY   = 0; //pong.cfg.wallWidth;
      this.maxY   = pong.height - this.height; //pong.height - pong.cfg.wallWidth - this.height;
  ...
  Ball: {
    ...
    initialize: function(pong) {
      ...
      this.minY    = this.radius; //pong.cfg.wallWidth + this.radius;
      this.maxY    = pong.height - this.radius; //pong.height - pong.cfg.wallWidth - this.radius;
  ...
  BackgroundNoise: {
    ...
    initialize: function(pong) {
      this.ping = true;
      this.canvas_noise = document.getElementById('noise');
      this.ctx_noise = this.canvas_noise.getContext('2d');
    },
    ...
    draw: function(ctx) {
      ...
      var buffer32 = new Uint32Array(image_data.data.buffer);
      ...
      if (this.ping) {
        gradient.addColorStop(0, "#6624ac");
        gradient.addColorStop(1, "#d14b66");
        ctx.fillStyle = gradient;
        ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);
        //ctx.fillStyle = "#1a19f3";
        //ctx.fillRect(0, 0, ctx.canvas.width / 2, ctx.canvas.height);
        //ctx.fillStyle = "#81d9db";
        //ctx.fillRect(ctx.canvas.width / 2, 0, ctx.canvas.width / 2, ctx.canvas.height);
      } else {
        gradient.addColorStop(0, "#d14b66");
        gradient.addColorStop(1, "#6624ac");
        ctx.fillStyle = gradient;
        ctx.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);
        //ctx.fillStyle = "#1a19f3";
        //ctx.fillRect(ctx.canvas.width / 2, 0, ctx.canvas.width / 2, ctx.canvas.height);
      ...
      this.ctx_noise.putImageData(image_data, 0, 0);
      this.ctx_noise.globalCompositeOperation = 'overlay';
      var grd = this.ctx_noise.createLinearGradient(ctx.canvas.width, ctx.canvas.height, ctx.canvas.width, ctx.canvas.height);
      grd.addColorStop(0, '#1a19f3');
      grd.addColorStop(1, '#81d9db');
      this.ctx_noise.fillStyle = grd;
      this.ctx_noise.fillRect(0, 0, ctx.canvas.width, ctx.canvas.height);
      this.ctx_noise.globalCompositeOperation = 'normal';
    }
